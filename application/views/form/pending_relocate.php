...
<script>
    ...
    var ci_prod_code = 1;
    var ci_prod_status_name = 3; //ADD BY POR 2013-12-19 เพิ่มคีย์ สำหรับไปแสดงใน PDF
    var ci_prod_sub_status_name = 4; //ADD BY POR 2013-12-19 เพิ่มคีย์ สำหรับไปแสดงใน PDF
    var ci_lot = 5;
    var ci_serial = 6;
    var ci_mfd = 7;
    var ci_exp = 8;
    ...
    //Add By por 2014-01-16 เพิ่มตัวแปร ราคาต่อหน่วย, หน่วย, ราคารวม
    ...
    var ci_unit_id = 22; //Edit by por 2014-01-16 แก้ไขจากเลข 14 เป็น 22 เนื่องจากมีการเพิ่ม column เลยทำให้คีย์เปลี่ยน
    var ci_item_id = 23; //Edit by por 2014-01-16 แก้ไขจากเลข 14 เป็น 23 เนื่องจากมีการเพิ่ม column เลยทำให้คีย์เปลี่ยน
    var ci_suggest_loc = 24; //Edit by por 2014-01-16 แก้ไขจากเลข 14 เป็น 24 เนื่องจากมีการเพิ่ม column เลยทำให้คีย์เปลี่ยน
    var ci_actual_loc = 25; //Edit by por 2014-01-16 แก้ไขจากเลข 14 เป็น 25 เนื่องจากมีการเพิ่ม column เลยทำให้คีย์เปลี่ยน
    var ci_unit_price_id = 26;      //Edit by por 2014-01-17 เพิ่มคีย์ รหัส unit price
    var ci_list = [
    ...
    function initProductTable() {
        var myDataTable = $('#showProductTable').dataTable({
            ...
            "bPaginate": false,
            "sScrollX": "100%",
//            "sScrollY": "300px", // Comment By Akkarapol, 11/11/2013, คอมเม้นต์ทิ้งเพราะเราจะให้มันใส่ความสูงแบบ auto ไปเลย
            "sScrollXInner": "120%",
            "sDom": '<"H"lfr>t<"F"ip>',
            "aoColumnDefs": [
                ...
                {"sWidth": "5%", "sClass": "center td_suggest_location", "aTargets": [16]},
                {"sWidth": "5%", "sClass": "center actual_loc_class", "aTargets": [17]},
                {"sWidth": "5%", "sClass": "left_text", "aTargets": [18]}
       
            ],
            "fnCreatedRow": function(nRow, aData, iDataIndex) {
                //$('td:eq(14)', nRow).attr('data-id', '1234');
            }
        }).makeEditable({
            ...
            "aoColumns": [
                null
                    ...
                    loadfirst: true, // Add By Akkarapol, 14/10/2013, เพิ่ม loadfirst เพื่อให้ช่อง Confirm Qty นี้มี textbox ขึ้นมาตั้งแต่โหลดหน้า
                    fnOnCellUpdated: function(sStatus, sValue, settings) {   // add fnOnCellUpdated for update total qty : by kik : 25-10-2013
                        calculate_qty();
                    }
                }
                ,null
                //ADD BY POR 2014-01-10 แสดงราคาต่อหน่วย, หน่วย, ราคารวม
	...
	curent_flow_action = $(elm).data('dialog');
        ...
        if (statusisValidateForm === true) {
            ...
            for (i in rowData) {
                ...
                // Comment By Akkarapol, 24/09/2013, Suggest Location ของทุกหน้า ไม่ต้อง require เพราะเดี๋ยวตอน เอาข้อมูลเก่ามาลง มันก็ไม่มีอยู่แล้ว
//                var suggest_loc = parseInt(rowData[i][ci_suggest_loc]);
//                if ("" == suggest_loc) {
//                    alert('Please Select Suggest Location');
//                    return false;
//                }
                // END Comment By Akkarapol, 24/09/2013, Suggest Location ของทุกหน้า ไม่ต้อง require เพราะเดี๋ยวตอน เอาข้อมูลเก่ามาลง มันก็ไม่มีอยู่แล้ว
            }
        ...
        $.post('<?php echo site_url() . "/reLocation/showProductInLocation" ?>', dataSet, function(data) {
            ...
            var oTable = $('#defDataTable').dataTable({
                ...
                "aoColumnDefs": [
                    ...
                    //{"sWidth": "5%", "sClass": "td_suggest_location center", "aTargets": [14]} //COMMENT BY POR 2013-12-13 เนื่องจากไม่มี column ที่ 14
                ]
            });
        }, "html");
...
// Comment By Akkarapol, 11/11/2013, เปลี่ยนการเรียกใช้ AutoComplete เป็นอีกแบบ เนื่องจากอันนี้ใช้งานได้ไม่ตรงจุด (แก้ defect : 451)

//    // Edit by Ton! 20131009
//    $(document).ready(function() {
//       
//        $('.suggest_loc_class').on('keyup focus', function(e) {//bind event td
//            var _this = this;
//            $('#suggestion_div').css({top: ($(this).offset().top + 30), left: $(this).offset().left}).show();
//            $.post("<?php echo site_url('/reLocation/showSuggestList'); ?>", {criteria: $(this).val(), status: $(this).data('status'), sub_status: $(this).data('sub_status'), category: $(this).data('category')}, function(response) {
...
//            });
//
//            $('.suggest_loc_class').find('input').blur(function() {//bind event td input
//                setTimeout(function() {
//                    $('#suggestion_div').hide();
//                }, 200);
//            });
...
//            $('.suggest_location_id').on('click', function(e) {//console.log(selector);
//                var tempCode = $(this).text();
//                var tempId = $(this).data('id');
////                var elm = $(selector);
////                var input = '<input class="' + elm.attr('class') + '" value="' + $(this).text() + '" data-status="' + elm.data('status') + '" data-sub_status="' + elm.data('sub_status') + '" data-category="' + elm.data('category') + '">';
...
//
//                setTimeout(function() {
//                    oTable.fnUpdate(tempCode, indexOfRow, 13); // update new data to datatables
////                    oTable.fnUpdate($(this).data('id'), indexOfRow, 18);
//                    var rowData = $('#showProductTable').dataTable().fnGetData();
//                    var num_row = rowData.length;
//                    if (num_row > 0) {
//                        rowData[indexOfRow][ci_suggest_loc] = tempId;
//                    }
//                }, 100);
//
//            });
...
//    
//    });
// END Comment By Akkarapol, 11/11/2013, เปลี่ยนการเรียกใช้ AutoComplete เป็นอีกแบบ เนื่องจากอันนี้ใช้งานได้ไม่ตรงจุด (แก้ defect : 451)
    ...
    // Add By Akkarapol, 11/11/2013, เพิ่ม Script ในส่วนของการทำ AutoComplete ที่ Column 'Suggest Location'
    $(document).ready(function() {

        function bindDataTD() {
            $('td.td_suggest_location').click(function() {
                var input = $('<input>').attr({'class': 'suggest_location'}).css('width', '95%').appendTo($(this));
                $(this).html(input);
                
                suggestLocation();
                input.focus(); //ADD BY POR 2013-12-13 เพิ่มให้สามารถ key data ได้
            });
...
//				var input = '<input class="'+elm.attr('class')+'" placeholder="'+elm.attr('placeholder')+'" value="'+$(this).text()+'" data-status="'+elm.data('status')+'" data-sub_status="'+elm.data('sub_status')+'" data-category="'+elm.data('category')+'">';
                ...
                oTable.fnUpdate(input, indexOfRow, ci_sugest_loc_name); // update new data to datatables
        ...
        function suggestLocation()
        {
            $('.suggest_location').on('keyup focus', function(e)
            {
                ...
                var move_qty = parseInt($(this).parent().prev().prev().prev().html());
                var balance_qty = parseInt($(this).parent().prev().prev().prev().prev().html());
                ...
                $.post("<?php echo site_url('/reLocation/show_suggest_list_by_product'); ?>", {criteria: $(this).val(), status: data['status'], sub_status: data['sub_status'], category: data['category'],item_id: data['item_id'],type: 2, qty: move_qty}, function(response)
                {
                    $('#suggestionLocationList').html(''); // clear old list 
                    $.each(response, function(index, value)
                    {
...
//        $('.td_suggest_location').click(function() {
...
//        });
    ...
    // END Add By Akkarapol, 11/11/2013, เพิ่ม Script ในส่วนของการทำ AutoComplete ที่ Column 'Suggest Location'
    ...
    function exportFile(file_type) {
        if (file_type == 'PDF') {
            //ADD BY POR 2013-12-19  เพิ่มให้ส่งค่าไปแบบ realtime
            ...
            //บอกให้รู้ว่ามาจากการ confirm ให้เอาค่า location จาก datatable เนื่องจากถ้าเป็น approve จะมาจาก database
            var v_method = document.createElement("input");
            v_method.setAttribute('type', "hidden");
            v_method.setAttribute('name', "method");
            v_method.setAttribute('value', 'confirm');
            f.appendChild(v_method);
...
<style>
    ...
    */
    ...
    /*Add by Akkarapol, 02/10/2013, เธ—เธณ autocomplete เน�เธ�เธชเน�เธงเธ�เธ�เธญเธ� Product Code เน�เธฅเธฐเน€เธกเธทเน�เธญเธ�เธฅเธดเธ�เธ—เธตเน� list product เธ”เธฑเธ�เธ�เธฅเน�เธฒเธง เธ�เน�เธ�เธฐเน�เธซเน� auto input to datatable*/
    .suggestionsBox {
        position: absolute;
        width: 200px;
    ...
    /* END Add by Akkarapol, 02/10/2013, เธ—เธณ autocomplete เน�เธ�เธชเน�เธงเธ�เธ�เธญเธ� Product Code เน�เธฅเธฐเน€เธกเธทเน�เธญเธ�เธฅเธดเธ�เธ—เธตเน� list product เธ”เธฑเธ�เธ�เธฅเน�เธฒเธง เธ�เน�เธ�เธฐเน�เธซเน� auto input to datatable*/

    /*Add By Akkarapol, 11/11/2013, เพิ่ม Stlye ของ Div Suggestion ให้แสดงผลได้สวยงาม*/
    #suggestion_div {
        width: 220px;
        height: 170px;
        display: none;
    ...
    /*END Add By Akkarapol, 11/11/2013, เพิ่ม Stlye ของ Div Suggestion ให้แสดงผลได้สวยงาม*/
...
<div class="well">
    ...
    <form id="form_pending" method='post' action="" target="_blank" >
        <fieldset class="well">
            <legend>&nbsp;Order&nbsp;</legend>
            <?php
            if (isset($flow_id)) {
			...
			<td align="left" style='display:none'>
					...
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                        <?php echo form_checkbox('is_repackage', ACTIVE, false); ?>&nbsp;Re-Package
			</td>
		  </tr>
	  </table>
	</fieldset>
						...
						<tr><?php echo $str_header; ?></tr>
                            ...
                            if (isset($pending_detail)) {
                                ...
                                foreach ($pending_detail as $order_column) {
                                    //ADD BY POR 2014-01-13 ถ้า confirm qty มีค่าแล้ว ให้แสดง allprice ตามค่าที่ส่งมา
...
//                                    $str_body .= "<td title=\"" . str_replace('"','&quot;',$order_column->Full_Product_Name) . "\">"; // Edit by Ton! 20140114
                                    $str_body .= "<tr title=\"" . str_replace('"','&quot;',$order_column->Full_Product_Name) . "\">"; // Edit by KIK! 20140115
...
//                                    $str_body .= "<td title='" . htmlspecialchars($order_column->Full_Product_Name, ENT_QUOTES) . "' >" . $order_column->Product_Name . "</td>";
                                    ...
                                    //ADD BY POR 2014-01-13 เพิ่ม column เกี่ยวกับ price
                                    ...
                                    $str_body .= "<td data-status='" . $order_column->Product_Status . "' data-sub_status='" . $order_column->Product_Sub_Status . "' data-category='" . $order_column->ProductCategory_Id . "' data-item_id='" . $order_column->Item_Id . "'>" . $order_column->Suggest_Location . "</td>";
...
<!--Add By Akkarapol, 11/11/2013, เพิ่ม Div Suggestion เพื่อรองรับการแสดงผล list ของ location ที่ถูกส่งมาผ่านทาง AutoComplete-->
<div id="suggestion_div">
    <div class="suggestionList" id="suggestionLocationList"></div>
</div>
<!-- END Add By Akkarapol, 11/11/2013, เพิ่ม Div Suggestion เพื่อรองรับการแสดงผล list ของ location ที่ถูกส่งมาผ่านทาง AutoComplete-->
