<script>
    ...
    $(document).ready(function () {
        ...
        var callback_count = (conf_cont ? 1 : 0);
        $.post('<?php echo site_url() . "/pre_receive/getProductStatus"; ?>', {"is_pending": "Y"}, function (data) {
            ...
            $.extend(master_product_status, JSON.parse(data));
              callback_count--;
                //  console.log(master_product_status);
        });
...
//        console.log(allVals);
    ...
    , {
            ...
            // var getdata_exp = getdata(value,rowIndex,function(data){
...
//    });
    ...
    var rowData2 = $('#showProductTable').dataTable().fnGetData();
                                    //// use fn in receive.php
        $.post('<?php echo site_url() . "/receive/check_last_mfd" ?>', { prod_mfd : mfd_data, prod_id : rowData2[rowIndex][17]}, function(result) {
            // alert(result);
            callback(result)
        }, "JSON");
    ...
    function getdata(mfd_data,rowIndex,callback) {
    var rowData2 = $('#showProductTable').dataTable().fnGetData();
    // console.log(rowData2);
    //  console.log(rowData2[rowIndex][17]);
    $.post('<?php echo site_url() . "/pre_receive/edit_mfd_to_exp" ?>', { data : mfd_data, prodid : rowData2[rowIndex][17]}, function(result) {
        callback(result)
    }, "JSON");
