...
<script>
    ...
    var redirect_after_save = site_url + "/reLocationProduct"; // เปลี่ยนให้เป็นตาม url ของหน้าที่ต้องการจะให้ redirect ไป โดยปกติจะเป็นหน้า list ของ flow นั้นๆ
...
//            } else {
        ...
        function actualLocation() {
            $('.actual_location').on('keyup focus', function(e) {
                ...
                // $.get("<?php echo site_url(); ?>" + "/reLocationProduct/test_nolocstion", {location_code: $(_this).val()}, function(result)
                //         {
                //         //    alert(result);exit();
                ...
                //                 $(_this).val($(_this).data('act_location2'));
                //                 error_location = -1; 
                //             } else {
                //                 error_count = 0;
                //                 error_location = 0;
                ...
                //           });
                 setTimeout(function() {
                    if ($(_this).val() != "")
                    {
                        ...
                        $.get("<?php echo site_url(); ?>" + "/reLocationProduct/get_location", {location_code: $(_this).val()}, function(result)
                        {
                        //    console.log(result);
                        // exit();
                            if (result != "null")  
        ...
        $('#search_submit').click(function() {
            ...
            $.post('<?php echo site_url() . "/reLocationProduct/showProduct" ?>', dataSet, function(data) {
                ...
                $.each(data.locations, function(i, item) {
                    ...
                    if(item.xml_move_qty == "TRUE"){
                        move_qty=item.Balance_Qty;
                        }      
                    ...
                    $('#showProductTable').dataTable().fnAddData([
                            recordsTotal
                                ...
                                , set_number_format(allprice)                  //add by kik : 20140115 suggesLocationClick(_this);
                                , item.Location_Code + ' <a href="javascript:;" ONCLICK="showProduct(\'\',\'' + item.Location_Code + '\')"><?php echo img("css/images/icons/view.png"); ?></a>'
                                , '<input type="text" class="required suggest_location" name="suggestion[]" placeholder="Search here.." value="'+item.Location_Code_master+'" data-inbound_id="'+item.Inbound_Id+'" data-status="' + item.Product_Status + '" data-sub_status="' + item.Product_Sub_Status_Code + '" data-category="'+item.Product_Category_Id+'" />' //'<span>Click to edit</span>'
