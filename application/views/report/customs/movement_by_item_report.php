<script>
    ...
    function exportFile(file_type) {
        if (file_type == 'EXCEL') {
            $("#form_report").attr('action', "<?php echo site_url(); ?>" + "/report_customs/exportExcelMovementAll_showItem");
        } else {
            $("#form_report").attr('action', "<?php echo site_url("/report_customs/export_movement_by_item"); ?>");
        }
        $("#form_report").submit();
    }
...
<form method="post" target="_blank" id="form_link_inven" action ='<?php echo site_url(); ?>/report/inventory'>
    ...
    <input type="hidden" name="product_id" id='product_id' value="<?php echo $form_value['product_id'];?>">
    ...
    <table id="defDataTable2" class="display dataTable" cellspacing="0" cellpadding="0" border="0" aria-describedby="defDataTable_info">
        <thead>
            <tr>
                ...
                <th><?php echo _lang('cus_customs_entry_inbound') ?></th>
                <th><?php echo _lang('number_dispatch') ?></th>
                <th><?php echo _lang('reference_dispatch') ?></th>
                <th><?php echo _lang('cus_customs_entry_outbound') ?></th>
                <th><?php echo _lang('cus_eor') ?></th>
...
//p($key); //exit();
                        ...
                        foreach ($values as $value) {
                            ...
                            ?>
                            <tr>
                                <td><?php echo '<font color=#DF0101><b>IOR</b>: '.$value['IOR_Name'].'</font>, '.$value['Product_Code'].'/'.$value['Product_NameEN']; ?></td>
                                <td><?php echo $i; ?></td>
                                <td><?php echo $value['receive_date']; ?></td>
                                <td>&nbsp;&nbsp;<?php echo $value['receive_doc_no']; ?></td>
                                <td><?php echo $value['receive_refer_ext']; ?></td>
                                <td><?php echo $value['CE_IN']; ?></td>
                                <td><?php echo $value['pay_doc_no']; ?></td>
                                <td><?php echo $value['pay_refer_ext']; ?></td>
                                <td><?php echo $value['CE_OUT']; ?></td>
                                <td><?php echo $value['EOR_Name']; ?></td>
                        ...
                        ?>
                            <tr>
                                <td class ="sum_group"><?php echo '<font color=#DF0101><b>IOR</b>: '.$value['IOR_Name'].'</font>, '.$value['Product_Code'].'/'.$value['Product_NameEN']; ?></td>
        ...
        <!-- show total qty : by kik : 31-10-2013-->
                    <tfoot>
                                ...
                                ?>
                             <tr>
                                    <th colspan="10" class ='ui-state-default indent'  style='text-align: center;'><b>Total Incoming Balance : <?php echo set_number_format(@$sumStartBalanceQty)?></b></th>
    ...
    <div align="center" style="margin:10px auto">
        ...
        <input type="hidden" name="product_id" id='product_id' value="<?php echo $form_value['product_id'];?>">
        <input type="hidden" name="product_name" id='product_name'  value="<?php echo $form_value['product_name'];?>">
        <input type="hidden" name="ior" value="<?php echo $form_value['ior'];?>">
        <input type="hidden" name="eor" value="<?php echo $form_value['eor'];?>">
        <input type="hidden" name="ce_in" value="<?php echo $form_value['ce_in'];?>">
        <input type="hidden" name="ce_out" value="<?php echo $form_value['ce_out'];?>">
    </div>
</form>