<script>
    ...
    function exportFile(file_type) {
        if (file_type == 'EXCEL') {
            $("#frmImportReport").attr('action', "<?php echo site_url(); ?>" + "/report_customs/exportImportToExcel");
        } else {
            $("#frmImportReport").attr('action', "<?php echo site_url(); ?>" + "/report_customs/exportImportToPDF");
        }
        $("#frmImportReport").submit();
    }
</script>
<style>
    td.group{
...
<form  method="post" target="_blank" id="frmImportReport">
                ...
                <table class ='well table_report' cellpadding="0" cellspacing="0" border="0" aria-describedby="defDataTable_info" style="max-width: none">
                    <thead>
                        <tr>
                            ...
                            <th style="width:100px;"><?php echo _lang('cus_customs_entry'); ?></th><!--เลขที่ใบขน/ใบขน-->
                            <th style="width:100px;"><?php echo _lang('invoice_no'); ?></th><!--เลขที่ INVOICE หรือเลขที่เอกสารประกอบ-->
                            <th style="width:100px;"><?php echo _lang('receive_date'); ?></th><!--วันนำเข้า/วันรับโอน-->
                            <th style="width:100px;"><?php echo _lang('product_code'); ?></th><!--รหัสสินค้า/วัตถุดิบ-->
                            <th style="width:200px;"><?php echo _lang('product_name'); ?></th><!--รายละเอียดสินค้า-->
                            <th style="width:80px;"><?php echo _lang('receive_qty'); ?></th><!--ปริมาณ-->
                            <th style="width:100px;"><?php echo _lang('unit'); ?></th>   <!--หน่วยนับ-->                 
                            <th style="width:80px;"><?php echo _lang('all_price'); ?></th> <!--มูลค่า-->
                        </tr>
                    </thead>
                    ...
                    <tbody>
                        ...
                        if(empty($data)):
                            ...
                            foreach ($data as $keyProduct => $datas) {
                                $i = 0;
                                ?>
                                <tr style="font-weight:bold;"><td style="background-color:#BBBBBB;">#</td><td style="background-color:#BBBBBB;text-align:left;" colspan=8>CE No.<?php echo $datas[0]['Doc_Refer_CE']; ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;IOR: <?php echo $datas[0]['Vendor_Name']; ?></td></tr>
                                <?php
                                $sum_receive = 0;
                                $sum_price = 0;
