...
if(!empty($datas['no_onhand'])){ 
    ...
    foreach ($datas as $keyProCode => $values) {
        ...
        //# getProductBalance() หา จำนวนสินค้าคงเหลือเริ่มต้นของสินค้าชิ้นนี้ 
        //# balance เก็บจำนวนสินค้าคงเหลือเริ่มต้น แล้วเอาไปคำนวนต่อใน loop
        //$balance=$this->r->getProductBalance($value[0]['product_id'],$search['fdate'],$search['tdate']);  
        //# start_balance สำหรับเอาไปแสดงตรงส่วน summary เมื่อสิ้นสุดตาราง
        //$start_balance=$balance;
        //$count_row=count($values);  #นับจำนวนรายการเคลื่อนไหวของสินค้าชิ้นนี้
        # max_line กำหนดจำนวนบรรทัดที่แสดงได้ใน 1 หน้า
        //$max_line=30;	
        # start calulate page number มีรายการเคลื่อนไหวหลายรายการต้องแบ่งการแสดงผลออกเป็นหลายหน้า ทำการคำนวนจำนวนหน้า
        //$totalpage = (int) ($count_row / $max_line);
        //if (($count_row % $max_line) != 0) {
        //		$totalpage += 1;
        //}
        # แสดงข้อมูบในแต่ละหน้า
        //for($i=1;$i<=$totalpage;$i++){    // loop show page
        //echo $page;exit();
        # คำนวนตำแหน่งของ array เริ่มตน และตำแหน่งสิ้นสุดที่จะดึงมาแสดงใน หน้านัน้ ๆ
        //	if($i==1){
        //		$start=0;
        //		$stop=$max_line-1;
        //	}
