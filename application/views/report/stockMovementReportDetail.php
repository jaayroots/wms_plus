...
function exportFile(file_type){
	if(file_type=='EXCEL'){
		$("#form_report").attr('action',"<?php echo site_url(); ?>"+"/report/exportExcelMovement")
	}else{
		$("#form_report").attr('action',"<?php echo site_url(); ?>"+"/report/exportReportStockMovementToPDF")
	}
	$("#form_report").submit();
 }
	...
	<tbody>
		<?php
		  ...
		  foreach($datas as $keyRow => $data){ 
				...
				<tr>
                                        <!-- Add By Akkarapol, 06/02/2014, ถ้าเปิดใช้งาน build_pallet และมี Pallet Code ให้ group ตัว Pallet Code แต่ถ้าไม่ ก็จะให้ group ด้วย Lot, Serial, Mdf, Exp เหมือนเดิม -->
					<?php if($this->config->item('build_pallet') and !empty($value['Pallet_Code'])): ?>
                                            <td>Pallet Code : <?php echo $value['Pallet_Code']; ?></td>
                                        <?php else: ?>
                                            <td>Lot/Serial : <?php echo $value['Product_SerLot'];?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Mfd : <?php echo $value['Product_Mfd'];?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Exp : <?php echo $value['Product_Exp'];?></td>
                                        <?php endif; ?>
                                    	<!-- END Add By Akkarapol, 06/02/2014, ถ้าเปิดใช้งาน build_pallet และมี Pallet Code ให้ group ตัว Pallet Code แต่ถ้าไม่ ก็จะให้ group ด้วย Lot, Serial, Mdf, Exp เหมือนเดิม -->
					...
					<td style='text-align: right'><?php echo set_number_format($value['r_qty']);?></td>
					<td style='text-align: right'><?php echo set_number_format($value['p_qty']);?></td>
			...
			$i++;
                        ...
                        $sumDispatchQty_inRow+=@$value['p_qty']; // add by kik : 25-11-2013
                                 ...
                                 <tr>
                                    ...
                                    <td class ="sum_group" style='text-align: right'><b><?php echo set_number_format(@$value['Balance_Qty']);?></b></td>
                                </tr>
