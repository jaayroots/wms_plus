    ...
    $.fn.makeEditable = function (options) {
        ...
        function _fnApplyEditable(aoNodes) {
            ...
            var oDefaultEditableSettings = {
                event: 'dblclick',
                "callback": function (sValue, settings, flag) {
                    properties.fnEndProcessingMode();
                    var status = "";
                    if (sNewCellValue == sValue) {
                        var aPos = oTable.fnGetPosition(this);                    	
                    	if (settings.type == "datepicker") {
                            oTable.fnUpdate(sNewCellValue, aPos[0], aPos[2]);                    		
                    	} else {
                ...
                "onsubmit": function (settings, original) {
                    ...
                    if (input.length == 1) {
                        ...
                        if (oEditElement.nodeName.toLowerCase() == "select" || oEditElement.tagName.toLowerCase() == "select") {
                        	...
                        	// By Ball
                        	// Add return default value if empty

                        	// Ball
                        	// 1. If empty value save previous value 
                        	/*if (sNewCellValue == "") {
                        		sNewCellDisplayValue = sOldValue;
                        	} else {
                        		sNewCellDisplayValue = sNewCellValue;
                        	}*/
                        	
                        	// Ball
                        	// 2. If empty value empty value
                        	if (sNewCellValue == "") {
